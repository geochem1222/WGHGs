...
const els = {
  ...
  ghgCount: document.querySelector("#ghg-count"),
...
function updateSourceLabel(data) {
  ...
  els.sourceCount.textContent = sources.join(" + ") || "Semantic Scholar + OpenAlex";
}
...
function updateStats() {
  ...
  els.ghgCount.textContent = state.papers.filter((paper) => {
    return ["co2", "ch4", "n2o", "ghg"].some((tag) => paper.tags.includes(tag));
  }).length;
}
