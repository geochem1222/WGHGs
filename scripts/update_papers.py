...
ENVIRONMENT_GROUPS = [
    "inland water inland waters freshwater",
...
BULK_SEARCH_QUERIES = [
    f"{environment} {gas}"
    for environment in BULK_ENVIRONMENT_TERMS
    for gas in BULK_GHG_TERMS
]
...
PUBMED_QUERY = """
(
  "greenhouse gas"[Title/Abstract] OR "greenhouse gases"[Title/Abstract]
  OR CO2[Title/Abstract] OR "carbon dioxide"[Title/Abstract]
  OR CH4[Title/Abstract] OR methane[Title/Abstract]
  OR N2O[Title/Abstract] OR "nitrous oxide"[Title/Abstract]
)
AND
(
  "inland water"[Title/Abstract] OR "inland waters"[Title/Abstract]
  ...
  OR channels[Title/Abstract] OR "water reservoir"[Title/Abstract] OR "water reservoirs"[Title/Abstract]
  OR "drinking water reservoir"[Title/Abstract] OR "reservoir lake"[Title/Abstract]
...
TAG_RULES = {
    "inland_water": ["inland water", "inland waters", "freshwater", "aquatic"],
    "river_stream": ["river", "rivers", "stream", "streams", "creek", "creeks"],
    "lake_reservoir": ["lake", "lakes", "water reservoir", "water reservoirs", "drinking water reservoir", "reservoir lake"],
    "pond_ditch": ["pond", "ponds", "ditch", "ditches", "canal", "canals", "channel", "channels"],
    "tidal": ["tidal creek", "tidal creeks", "tidal channel", "tidal channels", "estuary", "estuaries"],
    "wetland": ["wetland", "wetlands", "marsh", "marshes"],
    ...
    "microbial": ["microbial", "microbe", "microbes", "bacterial", "archaea", "methanogen", "methanotroph"],
    "urban": ["urban", "city", "cities", "stormwater", "wastewater", "sewage"],
    "agriculture": ["agriculture", "agricultural", "cropland", "farmland", "paddy", "irrigation"],
}
...
def build_user_agent(email: str | None) -> str:
    ...
    return f"wghgs-paper-tracker/1.0{contact}"
...
def enrich_with_semantic_metadata(
    ...
    candidates = [
        paper
        for paper in papers
        if (paper.get("doi") or paper.get("pmid") or paper.get("semantic_scholar_id") or paper.get("source") == "Semantic Scholar")
        and needs_metadata_refresh(paper, now, stale_days)
    ]
    if limit:
...
def apply_semantic_metadata(paper: dict[str, Any], item: dict[str, Any], now: datetime | None = None) -> None:
    external_ids = item.get("externalIds") or {}
    open_pdf = item.get("openAccessPdf") or {}
    doi = normalize_doi(external_ids.get("DOI", ""))
    ...
    paper["semantic_detail_enriched_at"] = (now or datetime.now(timezone.utc)).isoformat()
...
def enrich_with_semantic_recommendations(
    ...
    candidates = [
        ...
        if needs_recommendation_refresh(paper, now, stale_days)
    ][:limit]
    ...
    for paper in candidates:
        ...
        paper["semantic_recommendations_enriched_at"] = now.isoformat()
        time.sleep(0.35 if api_key else 1.05)

    return papers


def needs_metadata_refresh(paper: dict[str, Any], now: datetime, stale_days: int) -> bool:
    if not paper.get("semantic_scholar_id") or not paper.get("semantic_detail_enriched_at"):
        return True
    return is_stale(paper.get("semantic_detail_enriched_at"), now, stale_days)


def needs_recommendation_refresh(paper: dict[str, Any], now: datetime, stale_days: int) -> bool:
    if not paper.get("similar_papers") or not paper.get("semantic_recommendations_enriched_at"):
        return True
    return is_stale(paper.get("semantic_recommendations_enriched_at"), now, stale_days)
...
def existing_paper_ids(papers: list[dict[str, Any]]) -> set[str]:
    ids = set()
    for paper in papers:
        for key in ["semantic_scholar_id", "id"]:
            value = paper.get(key, "")
            if value and (paper.get("source") == "Semantic Scholar" or key == "semantic_scholar_id"):
                ids.add(value)
    return ids
